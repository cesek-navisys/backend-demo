import { FindFirstAccountProductQuery, FindProductByCodeQuery } from '@backend-demo/backend-libs/queries';
import { ProductReadService } from '../entity-layer/product-read.service';
import { IQueryHandler, QueryHandler } from '@nestjs/cqrs';

@QueryHandler(FindProductByCodeQuery)
export class QueryFindProductByCodeHandler implements IQueryHandler<FindProductByCodeQuery> {
    constructor(
...
export class QueryFindFirstProductHandler implements IQueryHandler<FindFirstAccountProductQuery> {
    constructor(
        private readonly productReadService: ProductReadService,
    ) { }
